...
<template>
    <header class="flex items-center gap-8 shadow-lg px-(--section-px) lg:px-(--section-px-lg) w-full h-full">
        <nav class="flex items-center cursor-pointer text-black w-full mx-auto max-w-7xl h-(--navbar-h) duration-300 ease-in-out transition-all">
            ...
            <div
                v-if="isMenuOpen"
                class="fixed top-(--navbar-h) flex inset-0 z-20"
            >
            ...
            <div
                v-if="isMenuOpen"
                class="fixed top-(--navbar-h) inset-0 z-20 transition-transform duration-300 ease-in-out"
            ...
            <div class="hidden lg:flex items-center justify-between font-bold text-sm gap-2 mx-auto cursor-pointer w-full max-w-7xl h-(--navbar-h) duration-300 ease-in-out transition-all">
                <RouterLink :to="{name: 'home'}" class="inline-flex items-center h-full px-5 hover:bg-purple-500 duration-300 ease-in-out transition-all">
                    Home
                </RouterLink>
